...
struct Minimo 
{
    ll minimo = INF;
    ll ocurrencias = 0;
    Minimo(ll min = INF, ll ocu = 0)
        : minimo(min), ocurrencias(ocu){}
};
...
struct SegTree
{
    ...
    Minimo op(Minimo a, Minimo b)
    {
        if(a.minimo==b.minimo){
            return Minimo(a.minimo, a.ocurrencias+b.ocurrencias);
        }

        return (a.minimo<b.minimo) ? a : b;
    }
    ...
    Minimo query(ll l, ll r)
    {
        return get(1, 0, arr.size() - 1, l, r);
    }

    Minimo get(ll node, ll sl, ll sr, ll l, ll r)
    {
        if (sr < l || r < sl)
            return Minimo();
        ...
        Minimo left = get(node * 2, sl, mid, l, r);
        Minimo right = get(node * 2 + 1, mid + 1, sr, l, r);
    ...
    */
    ...
    while(m--){
        ...
        if (comando == 1) {
            ...
            Minimo m = st.query(p1, p2-1);
            cout << m.minimo << ' ' << m.ocurrencias << '\n';
        }
    }
    return 0;
}